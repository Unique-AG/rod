...
const functions = {
  ...
  elementR(selector, regex) {
    ...
    const el = Array.from(s.querySelectorAll(selector)).find((e) => {
      const textFn = functions.text;
      // HACK!
      // there appears to be a rare case where `functions.text` is undefined here
      // which causes this function to throw.
      // I haven't been able to track down the underlying error, istd. we patch this
      // function to not throw if we encounter the error condition.
      if (typeof textFn !== 'function') {
        return false
      }

      return reg.test(textFn.call(e))
    });
