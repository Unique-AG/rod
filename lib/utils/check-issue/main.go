...
const ref = "\n_<sub>generated by [check-issue](https://github.com/Unique-AG/rod/tree/master/lib/utils/check-issue)</sub>_"
...
func main() {
	...
	if m == "" || m == "**Rod Version:** v0.0.0" {
		...
		q := req(fmt.Sprintf("/repos/Unique-AG/rod/issues/%d/comments", num))
...
func currentVer() string {
	q := req("/repos/Unique-AG/rod/tags?per_page=1")
	res, err := http.DefaultClient.Do(q)
	utils.E(err)
	defer func() { _ = res.Body.Close() }()
